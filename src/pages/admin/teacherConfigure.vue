<template>
  <div >
    <BreadCrumbs title="老师管理"></BreadCrumbs>
    <div class="program-list-head justify-between">
      <div class="add-program">
        <router-link :to="{path:'addTeacher'}">
          <a class="add-program-a">添加老师</a>
        </router-link>
      </div>
    </div>
    <div class="program-list-content">
      ...
      <div v-for="(teacher,index) in teacherList"  class="program-list-content-body justify-between">
        <div class="teacher-img"><img :src="teacher.photo"/></div>
        <div>{{teacher.teacherName}}</div>
        <div>{{teacher.educationName}}</div>
        <div>{{teacher.introduction | introductionFilter}}</div>
      </div>
      <div class="border-foot"></div>
      <QuickPager :page="page" @QuickPager="queryTeacherList"></QuickPager>
    </div>
  </div>
</template>
...
<script>
  ...
  export default{
    ...
    mounted: function () {
      axios.post('/free-api/getAllTeacher',{}).then((response)=>{
    ...
    methods: {
      queryTeacherList (page) {
        axios.post('/free-api/teacherList',page).then((response)=>{
          let rspCode = response.data.code;
          if (rspCode === '0') {
            let all=response.data.data;
